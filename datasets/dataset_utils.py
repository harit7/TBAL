...
def randomly_split_ds_idcs(ds,fraction=0.5,random_state=42) :
    n = ds.len()
    idcs = list(range(n))
    np.random.seed(random_state)
    idcs_val = np.random.choice(idcs,int(n*fraction),replace=False)
    idcs1_set = set(list(idcs_val))
    idcs_train = np.array( list(set(idcs).difference(idcs1_set) ))
    ds_val, ds_train     = ds.get_subset(idcs_val), ds.get_subset(idcs_train)
    #print(len(idcs1),len(idcs2)) 
    return {'idcs_std_train': idcs_train,'idcs_std_val':idcs_val,'ds_std_train':ds_train,'ds_std_val':ds_val}
