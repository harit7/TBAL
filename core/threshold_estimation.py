...
def determine_threshold(lst_classes,inf_out,auto_lbl_conf,val_ds,val_idcs,logger,err_threshold=0.01):
    ...
    if(fast and len(scores)>500):
        ...
        delta = (max_score-min_score)/10000
    ...
    S[:,5] = inf_out['abs_logit']
    
    if('energy_score' in inf_out):
        S[:,6] = inf_out['energy_score']

    scores_id_map = {'confidence':4,'abs_logit':5,'energy_score':6}
    ...
    return lst_t_y, val_idcs_to_rm, val_err
